  ...
  it('should create a new group', async () => {
    const { data, error } = await supabase
      ...
      .insert({
        ...
        emoji: '🧪',
        currency: 'USD',
        share_code: 'TEST' + Math.random().toString(36).substring(2, 4).toUpperCase(),
      })
      .select()
      .single();
...
describe('Expenses CRUD', () => {
  it('should create an expense', async () => {
    ...
    expect(parseFloat(data.amount)).toBe(90.00);
...
describe('Cascade Deletes', () => {
  ...
  beforeAll(async () => {
    ...
    await supabase
      .from('splits')
      .insert({ expense_id: tempExpenseId, member_id: tempMemberId, amount: 10 });
  });
